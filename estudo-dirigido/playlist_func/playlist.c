...
static EntradaPlaylist *entrada_buscar(const NodoPlaylist *pl,
                                       const NodoMusica   *musica) {
    EntradaPlaylist *e = pl->inicio_musicas;
    while (e) {
        if (e->musica == musica) return e;
        e = e->prox;
    }
    return NULL;
}
...
static void entrada_desencadear(NodoPlaylist *pl, EntradaPlaylist *e) {
    if (e->ant) e->ant->prox = e->prox;
    else        pl->inicio_musicas = e->prox;

    if (e->prox) e->prox->ant = e->ant;
    else         pl->fim_musicas = e->ant;
...
int playlist_remover(ListaPlaylists *lp, NodoPlaylist *alvo) {
    ...
    if (alvo->ant) alvo->ant->prox = alvo->prox;
...
NodoMusica *playlist_proxima(const NodoPlaylist *pl, const NodoMusica *atual) {
    ...
    while (e) {
        if (e->musica == atual) {
            return e->prox ? e->prox->musica : NULL;
        }
        e = e->prox;
    }
...
NodoMusica *playlist_anterior(const NodoPlaylist *pl, const NodoMusica *atual) {
    ...
    while (e) {
        if (e->musica == atual) {
            return e->ant ? e->ant->musica : NULL;
        }
        e = e->prox;
    }
    return NULL; /* atual nao pertence a esta playlist */
...
int playlist_em_alguma(const void *lista_playlists, const NodoMusica *musica) {
    ...
    while (pl) {
        if (entrada_buscar(pl, musica)) return 1;
        pl = pl->prox;
    }
    return 0;
}
